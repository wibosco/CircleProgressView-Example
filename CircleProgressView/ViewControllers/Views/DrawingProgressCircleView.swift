//
//  DrawingProgressCircleView.swift
//  CircleProgressView
//
...
@IBDesignable
class DrawingProgressCircleView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let endAngle = (progress * (2 * CGFloat.pi)) - (CGFloat.pi/2)
        
        contextRef.setLineWidth(lineWidth)
        contextRef.addArc(center: center, radius: radius, startAngle: -(CGFloat.pi/2), endAngle: endAngle, clockwise: false)
        contextRef.setStrokeColor(progressColor.cgColor)
        contextRef.setLineCap(.round)
        contextRef.strokePath()
    }
}
